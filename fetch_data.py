...
def fetch_scores_for_standings():
    """
    atplayertw.com.tw/cpbl/ 實際結構（從 debug log 確認）：
    table[0]: 球隊, W, L, PCT, GB, 得, 失, 分差, 連勝敗, 近況  → 積分榜
    ...
    try:
        ...
        if len(tables) >= 1:
            for row in tables[0].find_all('tr')[1:]:
                ...
                if len(cols) < 4:
                    continue
                team = None
                for t in ['味全龍','統一獅','富邦悍將','台鋼雄鷹','樂天桃猿','中信兄弟']:
                    if t in cols[0]:
                        team = t; break
                if not team:
                    continue
                try:
                    ...
                    streak = ''
                    if len(cols) >= 9:
                        recent_str = cols[-1].strip()
                        if recent_str:
                            last = recent_str[-1]
                            cnt = sum(1 for c in reversed(recent_str) if c == last)
                            streak = f"{'W' if last=='W' else 'L'}{min(cnt,5)}"
                except:
                    continue
                standings_override = standings_override or []
                standings_override.append({
                    'team': team, 'win': w, 'loss': l, 'draw': 0,
                    'pct': pct_raw, 'gb': gb, 'streak': streak
                })

            if standings_override:
                standings_override.sort(key=lambda x: (-x['win'], x['loss']))
                standings_override = [dict(r, rank=i+1) for i, r in enumerate(standings_override)]
                print(f"  → 積分榜：{len(standings_override)} 隊")

        # ── 比賽結果（table[1]）: 台灣時間, 客隊, 比分, 主隊, 狀態, 場地 ──
        ...
        for r in results[:5]:
            print(f"    {r['date']} {r['away']} {r['away_score']}:{r['home_score']} {r['home']}")
