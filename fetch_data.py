...
def fetch_scores_for_standings():
    """
    disp.cc 戰績貼文結構（已確認）：
    - 同主題連結列表格式：「05-19 21:35 nybbnan. ■ [分享] CPBL戰績 /b/Baseball/i4d5」
    - 比分格式：「台鋼雄鷹 0:2 味全龍」（一行一場）
    - 積分榜：「1 味全龍 35 22-0-13 0.629 0.0 W4」
    策略：抓最新貼文，從同主題連結取過去 10 天的 URL，逐篇解析
    """
    results = []
    standings_override = None
    seen = set()
    TEAMS = ['味全龍','統一獅','富邦悍將','台鋼雄鷹','樂天桃猿','中信兄弟']

    def parse_one(url, date_str):
        """解析單篇戰績貼文，回傳 (standings_rows, game_results)"""
        rows, games = [], []
        try:
            soup = fetch_soup(url)
            text = soup.get_text('\n')
            for line in [l.strip() for l in text.splitlines() if l.strip()]:
                ...
                if gm and date_str:
                    ...
                    if a_s<=25 and h_s<=25:
                        ...
                        if key not in seen:
                            seen.add(key)
                            games.append({'date':date_str,'away':away,'away_score':a_s,
                                          'home':home,'home_score':h_s})
        except Exception as e:
            print(f"  ⚠️ {url}: {e}")
        return rows, games

    # 1. 抓最新貼文，從同主題連結列表取所有近期 URL
    try:
        soup0 = fetch_soup('https://disp.cc/b/Baseball/i4d5')
        text0 = soup0.get_text('\n')

        # 找同主題連結：格式「05-19 21:35 nybbnan. ■ [分享] CPBL戰績 /b/Baseball/XXXX」
        # 在 HTML 中是 <a href="/b/Baseball/XXXX">
        thread_articles = []  # list of (date_str, url)
        for a in soup0.find_all('a', href=re.compile(r'^/b/Baseball/[a-zA-Z0-9]+$')):
            href = a['href']
            url_full = 'https://disp.cc' + href
            # 找這個連結附近的日期（前後文字）
            parent_text = a.parent.get_text() if a.parent else ''
            dm = re.search(r'(\d{2})-(\d{2})', parent_text)
            if dm and '戰績' in parent_text and '二軍' not in parent_text:
                month, day = int(dm.group(1)), int(dm.group(2))
                y = CUR_YEAR
                art_date = f"{y}/{month:02d}/{day:02d}"
                thread_articles.append((art_date, url_full))

        # 也加入今天最新的貼文本身
        thread_articles.append((TODAY.strftime('%Y/%m/%d'), 'https://disp.cc/b/Baseball/i4d5'))

        # 去重，排序，只取最近 10 天
        import datetime as dt
        cutoff = (TODAY - dt.timedelta(days=10)).strftime('%Y/%m/%d')
        unique_articles = {}
        for date_str, url in thread_articles:
            if date_str >= cutoff and url not in unique_articles:
                unique_articles[url] = date_str

        print(f"  → 找到 {len(unique_articles)} 篇戰績貼文")

        # 2. 逐篇解析
        for url, date_str in sorted(unique_articles.items(), key=lambda x: x[1], reverse=True):
            rows, games = parse_one(url, date_str)
            results.extend(games)
            if rows and not standings_override:
                standings_override = sorted(rows, key=lambda x: x['rank'])
                print(f"  → 積分榜：{len(standings_override)} 隊")

        # 如果最新貼文沒有，試今天的（可能是新 URL）
        if not standings_override:
            # 搜尋 disp.cc Baseball 板找今天的戰績貼文
            try:
                soup_board = fetch_soup('https://disp.cc/b/Baseball')
                for a in soup_board.find_all('a', href=re.compile(r'/b/Baseball/')):
                    if 'CPBL戰績' in a.get_text() or 'CPBL 戰績' in a.get_text():
                        new_url = 'https://disp.cc' + a['href']
                        rows2, games2 = parse_one(new_url, TODAY.strftime('%Y/%m/%d'))
                        results.extend(games2)
                        if rows2:
                            standings_override = sorted(rows2, key=lambda x: x['rank'])
                        break
            except:
                pass

    except Exception as e:
        print(f"  ⚠️ disp.cc 主貼文失敗: {e}")
