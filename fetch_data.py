...
def fetch_scores_for_standings():
    """
    從 disp.cc 戰績貼文抓積分榜和近期比賽。
    同主題文章列表在貼文底部，格式：
      05-20 22:00 nybbnan. ■ [分享] CPBL戰績 → href=/b/Baseball/i4hS
    """
    results = []
    ...
    try:
        # 抓任一已知的戰績貼文，從中取同主題連結列表
        soup0 = fetch_soup('https://disp.cc/b/Baseball/i4d5')
        full_text = soup0.get_text('\n')

        # 同主題連結在文字中格式：「05-20 22:00 nybbnan. ■ [分享] CPBL戰績 /b/Baseball/i4hS」
        # 同時在 HTML 中有 <a href="/b/Baseball/XXXX">
        # 用文字匹配找日期+URL
        articles = {}  # url -> date_str

        # 方法1：從文字找「MM-DD ... CPBL戰績 ... /b/Baseball/XXXX」
        lines = full_text.splitlines()
        for i, line in enumerate(lines):
            if 'CPBL戰績' in line and '二軍' not in line:
                # 找日期
                dm = re.search(r'(\d{2})-(\d{2})', line)
                # 找 URL（在同行或下幾行）
                for j in range(i, min(i+3, len(lines))):
                    um = re.search(r'/b/Baseball/([a-zA-Z0-9]+)', lines[j])
                    if um and dm:
        ...
        # 方法2：從 HTML <a> 標籤找，搭配父元素文字取日期
        for a in soup0.find_all('a', href=re.compile(r'/b/Baseball/[a-zA-Z0-9]+')):
            href = a['href']
            art_url = f"https://disp.cc{href}" if href.startswith('/') else href
            ...
            for _ in range(3):
                if p:
                    ctx = p.get_text()
                    p = p.parent
                    if re.search(r'\d{2}-\d{2}', ctx) and ('CPBL戰績' in ctx or 'CPBL 戰績' in ctx):
                        break
            dm2 = re.search(r'(\d{2})-(\d{2})', ctx)
            if dm2 and ('CPBL戰績' in ctx or 'CPBL 戰績' in ctx) and '二軍' not in ctx:
