...
def fetch_scores_for_standings():
    """
    disp.cc CPBL戰績貼文（每天晚上有人發）
    格式：積分榜 + 當天比分（如「台鋼雄鷹 0:2 味全龍」）
    策略：抓最新一篇，再追同主題文章往前抓 10 天
    """
    results = []
    ...
    def parse_article(url, article_date):
        """解析一篇戰績貼文，回傳 (standings_rows, game_results)"""
        rows, games = [], []
        try:
            soup = fetch_soup(url)
            text = soup.get_text('\n')
            ...
            for line in lines:
                # 積分榜行：「1 味全龍 35 22-0-13 0.629 0.0 W4」
                m = re.match(
                ...
                if gm and article_date:
                    away, a_s, h_s, home = gm.group(1), int(gm.group(2)), int(gm.group(3)), gm.group(4)
                    key = f"{article_date}{away}{home}"
                    if key not in seen and a_s <= 25 and h_s <= 25:
                        seen.add(key)
                        games.append({'date': article_date, 'away': away,
                                      'away_score': a_s, 'home': home, 'home_score': h_s})

            # 找同主題文章連結（更早的戰績）
            thread_links = []
            for a in soup.find_all('a', href=re.compile(r'/b/Baseball/')):
                href = 'https://disp.cc' + a['href'] if a['href'].startswith('/') else a['href']
                if 'disp.cc/b/Baseball/' in href and href not in thread_links:
                    thread_links.append(href)

        except Exception as e:
            print(f"  ⚠️ {url} 解析失敗: {e}")
        return rows, games, thread_links

    # 動態找最新戰績貼文（搜尋 disp.cc Baseball 板 CPBL戰績）
    SEARCH_URL = 'https://disp.cc/b/Baseball'
    seed_url = None
    try:
        soup0 = fetch_soup(SEARCH_URL)
        # 找標題包含「CPBL戰績」的文章連結
        for a in soup0.find_all('a', href=re.compile(r'/b/Baseball/')):
            text_a = a.get_text(strip=True)
            if 'CPBL戰績' in text_a or 'CPBL 戰績' in text_a:
                href = a['href']
                if not href.startswith('http'):
                    href = 'https://disp.cc' + href
                seed_url = href
                break
    except Exception as e:
        print(f"  ⚠️ 搜尋戰績貼文失敗: {e}")

    if not seed_url:
        seed_url = 'https://disp.cc/b/Baseball/i4d5'  # fallback: 5/19 戰績
    print(f"  → 戰績貼文起點: {seed_url}")

    import datetime as dt
    cutoff = (TODAY - dt.timedelta(days=10)).strftime('%Y/%m/%d')
    visited = set()
    queue = [(seed_url, TODAY.strftime('%Y/%m/%d'))]

    for _ in range(12):  # 最多抓 12 篇
        if not queue:
            break
        url, article_date = queue.pop()
        if url in visited:
            continue
        visited.add(url)

        rows, games, thread_links = parse_article(url, article_date)
        results.extend(games)

        # 最新一篇的積分榜作為主要積分榜
        if rows and not standings_override:
            standings_override = sorted(rows, key=lambda x: x['rank'])
            print(f"  → 積分榜：{len(standings_override)} 隊（來自 {url}）")

        # 把同主題的舊文加入 queue
        for lk in reversed(thread_links):
            if lk not in visited and 'Baseball' in lk:
                # 從文章日期往前推算（每篇約差1天）
                prev_date_str = article_date  # 保守用同一天，實際比分日期在內文解析
                if lk not in [u for u, _ in queue]:
                    queue.insert(0, (lk, prev_date_str))
