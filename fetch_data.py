...
def fetch_scores_for_standings():
    """
    atplayertw 真實結構（已確認）:
    table[0]: 球隊, W, L, PCT, GB, 得, 失, 分差, 連勝敗, 近況 → 積分榜（無平手）
    table[1]: 台灣時間, 客隊, 比分, 主隊, 狀態, 場地 → 比賽結果
    平手資料要從 CPBL schedule 頁面取得
    """
    results = []
    ...
    try:
        ...
        if tables:
            ...
            for row in tables[0].find_all('tr')[1:]:
                ...
                if len(cols) < 4:
                    continue
                team = next((t for t in ['味全龍','統一獅','富邦悍將','台鋼雄鷹','樂天桃猿','中信兄弟'] if t in cols[0]), None)
                if not team:
                    continue
                try:
                    ...
                    # 近況（最後一欄）算連勝/連敗
                    streak = ''
                    near = cols[-1].strip() if cols else ''
                    if near and re.fullmatch(r'[WL]+', near):
                        last = near[-1]
                        cnt = sum(1 for c in reversed(near) if c == last)
                        streak = f"{'W' if last=='W' else 'L'}{min(cnt,5)}"
                    # 連勝敗欄（col[8]）: '勝2' or '敗1'
                    elif len(cols) > 8:
                        streak_text = cols[8].strip()
                        m = re.match(r'(勝|敗)(\d+)', streak_text)
                        if m:
