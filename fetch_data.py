...
def fetch_scores_for_standings():
    """
    積分榜 + 近期比賽：從 disp.cc CPBL戰績貼文系列抓取
    每天晚上有人發貼，格式固定：
    ...
    all_games = []    # 所有近期比賽
    seen_keys = set()
    standings_done = False  # 只取最新一篇的積分榜

    cutoff = (TODAY - dt.timedelta(days=10)).strftime('%Y/%m/%d')

    TEAM6 = ['味全龍','統一獅','富邦悍將','台鋼雄鷹','樂天桃猿','中信兄弟']

    def parse_article(url, art_date):
        nonlocal standings_done
        rows, games = [], []
        try:
            soup = fetch_soup(url)
            for line in [l.strip() for l in soup.get_text('\n').splitlines() if l.strip()]:
                # 積分榜行
                if not standings_done:
                    m = re.match(
                ...
                if gm and art_date and art_date >= cutoff:
                    away,a_s,h_s,home = gm.group(1),int(gm.group(2)),int(gm.group(3)),gm.group(4)
                    if a_s <= 25 and h_s <= 25:
                        key = f"{art_date}{away}{home}"
                        if key not in seen_keys:
                            seen_keys.add(key)
                            games.append({'date':art_date,'away':away,'away_score':a_s,
    ...
    # ── 步驟1：找所有近期戰績貼文 URL ──
    articles = {}  # url -> date_str

    # 從已知種子貼文的同主題文章列表取得其他連結
    seed_urls = [
        'https://disp.cc/b/Baseball/i4d5',  # 5/19
    ]
    ...
    try:
        ...
        for a in soup_b.find_all('a', href=re.compile(r'/b/Baseball/[a-zA-Z0-9]+')):
            ...
            if 'CPBL戰績' in txt or 'CPBL戰績' in parent_txt:
                art_url = 'https://disp.cc' + a['href']
                # 日期從父元素找
                dm = re.search(r'(\d{2})-(\d{2})', parent_txt)
                if dm:
                    art_date = f"{CUR_YEAR}/{int(dm.group(1)):02d}/{int(dm.group(2)):02d}"
                    articles[art_url] = art_date
                    if art_url not in seed_urls:
                        seed_urls.insert(0, art_url)
                    break
    except Exception as e:
        print(f"  ⚠️ 板頁搜尋失敗: {e}")
    ...
    for seed in seed_urls[:2]:
        try:
            soup_s = fetch_soup(seed)
            full_text = soup_s.get_text('\n')
            lines_s = full_text.splitlines()

            for i, line in enumerate(lines_s):
                if ('CPBL戰績' in line or 'CPBL 戰績' in line) and '二軍' not in line:
                    dm = re.search(r'(\d{2})-(\d{2})', line)
                    for j in range(max(0,i-1), min(len(lines_s),i+3)):
                        um = re.search(r'/b/Baseball/([a-zA-Z0-9]+)', lines_s[j])
                        if um and dm:
    ...
    # ── 步驟2：逐篇解析，最新的先 ──
    for url, art_date in sorted(articles.items(), key=lambda x: x[1], reverse=True):
        rows, games = parse_article(url, art_date)
        all_games.extend(games)
        # 積分榜只用最新一篇（第一篇能解析到6隊就標記完成）
        if rows and not standings_done:
            all_rows.extend(rows)
            if len(all_rows) >= 6:
                standings_done = True
                print(f"  → 積分榜：{len(all_rows)} 隊（來自 {url}）")

    standings_override = sorted(all_rows, key=lambda x: x['rank']) if all_rows else None
    print(f"  → 近期比賽共 {len(all_games)} 場")
    return all_games, standings_override
