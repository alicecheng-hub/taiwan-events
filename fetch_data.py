...
def fetch_scores_for_standings():
    """
    從 en.cpbl.com.tw/standings 抓積分榜（含平手）
    從 en.cpbl.com.tw/box 抓近期7天比分
    en.cpbl.com.tw 是純 SSR，不擋 GitHub Actions IP
    回傳 (game_results, standings_override)
    """
    results = []
    ...
    # ── 積分榜 ──
    try:
        soup = fetch_soup('https://en.cpbl.com.tw/standings')
        EN_TO_ZH = {
            'BROTHERS': '中信兄弟', 'U-LIONS': '統一獅', 'MONKEYS': '樂天桃猿',
            'GUARDIANS': '富邦悍將', 'DRAGONS': '味全龍', 'TSG': '台鋼雄鷹',
            'Brothers': '中信兄弟', 'Lions': '統一獅', 'Monkeys': '樂天桃猿',
            'Guardians': '富邦悍將', 'Dragons': '味全龍', 'Hawks': '台鋼雄鷹',
        }
        rows = []
        for table in soup.find_all('table'):
            headers = [th.get_text(strip=True) for th in table.find_all('th')]
            # 找有 W L TIE PCT 的表格
            if not ('W' in headers and 'L' in headers and 'PCT' in headers):
                continue
            tie_idx = headers.index('TIE') if 'TIE' in headers else None
            pct_idx = headers.index('PCT') if 'PCT' in headers else None
            gb_idx  = next((i for i,h in enumerate(headers) if 'GB' in h or '差' in h), None)
            ...
            for row in table.find_all('tr')[1:]:
                ...
                if not team:
                    # 試中文直接匹配
                    for zh in ['中信兄弟','統一獅','樂天桃猿','富邦悍將','味全龍','台鋼雄鷹']:
                        if zh in cols[0]:
                            team = zh; break
                if not team:
                    continue
                try:
                    # 找 W L TIE
                    nums = [c.strip() for c in cols if re.fullmatch(r'\d+', c.strip())]
                    if len(nums) < 2:
                        continue
                    w = int(nums[0])
                    l = int(nums[1])
                    tie = int(nums[2]) if len(nums) > 2 else 0
                    # PCT
                    pct_raw = cols[pct_idx].strip() if pct_idx and pct_idx < len(cols) else ''
                    if not pct_raw:
                        pct_float = w / (w + l + tie) if (w + l + tie) > 0 else 0
                        pct_raw = f".{int(pct_float * 1000):03d}"
                    elif '.' in pct_raw and not pct_raw.startswith('.'):
                        pct_raw = '.' + pct_raw.split('.')[-1]
                    # GB
                    gb = cols[gb_idx].strip() if gb_idx and gb_idx < len(cols) else '-'
                    if gb == '0' or gb == '0.0':
                        gb = '-'
                except:
                    continue
                rows.append({'team': team, 'win': w, 'loss': l, 'draw': tie,
                             'pct': pct_raw, 'gb': gb, 'streak': ''})

            if rows:
                rows.sort(key=lambda x: (-x['win'], x['loss']))
                standings_override = [dict(r, rank=i+1) for i, r in enumerate(rows)]
                print(f"  → en.cpbl 積分榜：{len(standings_override)} 隊")
                break

    except Exception as e:
        print(f"  ⚠️ en.cpbl 積分榜失敗: {e}")

    # 若英文站抓不到，改用 atplayertw
    if not standings_override:
        try:
            soup2 = fetch_soup('https://atplayertw.com.tw/cpbl/')
            TEAM_EN_MAP = {
                '味全龍':'味全龍','統一獅':'統一獅','富邦悍將':'富邦悍將',
                '台鋼雄鷹':'台鋼雄鷹','樂天桃猿':'樂天桃猿','中信兄弟':'中信兄弟',
            }
            rows2 = []
            for table in soup2.find_all('table'):
                headers = [th.get_text(strip=True) for th in table.find_all('th')]
                if 'W' not in headers or 'L' not in headers:
                    continue
                for row in table.find_all('tr')[1:]:
                    cols = [td.get_text(' ', strip=True) for td in row.find_all('td')]
                    if len(cols) < 4:
                    ...
                    try:
                        ...
                        pct_s = '.' + cols[3].strip().split('.')[-1]
                        gb = cols[4].strip() if len(cols) > 4 else '-'
                    except:
                        continue
                    rows2.append({'team':team,'win':w,'loss':l,'draw':0,'pct':pct_s,'gb':gb,'streak':''})
                if rows2:
                    rows2.sort(key=lambda x: (-x['win'], x['loss']))
                    standings_override = [dict(r, rank=i+1) for i, r in enumerate(rows2)]
                    print(f"  → atplayertw 積分榜：{len(standings_override)} 隊")
                    break
        except Exception as e:
            print(f"  ⚠️ atplayertw 積分榜也失敗: {e}")
    ...
    seen = set()
    try:
        soup3 = fetch_soup('https://atplayertw.com.tw/cpbl/')
        for table in soup3.find_all('table'):
            ttext = table.get_text()
            if '客隊' not in ttext and '比分' not in ttext:
                continue
            for row in table.find_all('tr')[1:]:
                cols = [td.get_text(' ', strip=True) for td in row.find_all('td')]
                if len(cols) < 3:
                    continue
                full = ' '.join(cols)
                if '尚未開打' in full or '進行中' in full or 'vs.' in full.lower():
                    continue
                date_m = re.search(r'(\d{1,2})/(\d{1,2})', cols[0])
                if not date_m:
                    continue
                gd = f"{CUR_YEAR}/{int(date_m.group(1)):02d}/{int(date_m.group(2)):02d}"
                if not is_past(gd) or gd < cutoff:
                    continue
                # 比分必須兩個數字都 ≤ 25，排除時間（18:35）
                score_m = re.search(r'\b(\d{1,2})\s*[：:]\s*(\d{1,2})\b', full)
                if not score_m:
                    continue
        ...
        print(f"  → 近期比賽：{len(results)} 場")
    except Exception as e:
        print(f"  ⚠️ 近期比賽抓取失敗: {e}")
