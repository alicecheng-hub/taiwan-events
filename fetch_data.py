...
def fetch_scores_for_standings():
    """
    從 disp.cc PTT Baseball 板「CPBL戰績」系列貼文抓積分榜和近期比賽。
    格式：RK TEAM G W-T-L PCT GB STRK
    每天有人更新，包含平手、差距、連勝敗、當日比分。
    策略：抓最新一篇（從串列頁找最後一篇），解析文章內容。
    """
    results = []
    ...
    try:
        # 1. 抓最新一篇的 URL（從任何一篇的同主題文章列表取最後一篇）
        # 用已知的一篇作為入口
        ...
        # 找同主題文章連結中最新的（日期最大的）
        import re as _re
        thread_links = re.findall(r'(https://disp\.cc/b/Baseball/[a-zA-Z0-9]+)', text)
        # 過濾重複，取最後幾個（最新的在後）
        seen_links = []
        for lk in thread_links:
            if lk not in seen_links:
                seen_links.append(lk)
        latest_url = seen_links[-1] if seen_links else 'https://disp.cc/b/Baseball/i3YR'

        # 2. 抓最新貼文內容
        if latest_url != 'https://disp.cc/b/Baseball/i3YR':
            soup = fetch_soup(latest_url)
            text = soup.get_text('\n')
            lines = [l.strip() for l in text.splitlines() if l.strip()]

        print(f"  → disp.cc 最新URL: {latest_url}")

        # 3. 解析積分榜
        # 格式行：「1 味全龍 33 20-0-13 0.606 0.0 W2」
        rows = []
        for line in lines:
            m = re.match(
                r'^(\d)\s+(味全龍|統一獅|富邦悍將|台鋼雄鷹|樂天桃猿|中信兄弟)\s+'
                r'\d+\s+(\d+)-(\d+)-(\d+)\s+([\d.]+)\s+([\d.]+|-)\s+([WL]\d+)',
                line
            )
            if m:
                rk, team, w, tie, l, pct, gb, strk = m.groups()
                pct_fmt = '.' + pct.split('.')[-1] if '.' in pct else pct
                gb_fmt = '-' if gb == '0.0' or gb == '-' or rk == '1' else gb
                rows.append({
                    'rank': int(rk), 'team': team,
                    'win': int(w), 'draw': int(tie), 'loss': int(l),
                    'pct': pct_fmt, 'gb': gb_fmt, 'streak': strk
                })
        ...
        if rows:
            ...
            for s in standings_override:
                print(f"    {s['rank']}. {s['team']} {s['win']}-{s['draw']}-{s['loss']} {s['pct']} GB:{s['gb']} {s['streak']}")
        ...
        for line in lines:
            ...
            if sm:
                away, a_s, h_s, home = sm.group(1), int(sm.group(2)), int(sm.group(3)), sm.group(4)
                key = f"{today_str}{away}{home}"
                if key not in seen:
                    seen.add(key)
                    results.append({
                        'date': today_str,
                        'away': away, 'away_score': a_s,
                        'home': home, 'home_score': h_s,
                    })

        # 5. 也從 atplayertw 補近期幾天比分
        try:
            ...
            print(f"  ⚠️ atplayertw 比賽補充失敗: {e2}")
