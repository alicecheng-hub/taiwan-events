...
def fetch_scores_for_standings():
    """
    從 atplayertw.com.tw/cpbl/ 抓積分榜和近期比賽。
    加入大量 debug 輸出方便排錯。
    """
    results = []
    ...
    try:
        ...
        print(f"  → atplayertw 找到 {len(tables)} 個 table")
        ...
        print(f"  ⚠️ atplayertw debug 失敗: {e}")

    # ── 積分榜：嘗試解析 ──
    try:
        soup = fetch_soup('https://atplayertw.com.tw/cpbl/')

        TEAM_ZH = ['味全龍','統一獅','富邦悍將','台鋼雄鷹','樂天桃猿','中信兄弟']
        rows_data = []

        for table in soup.find_all('table'):
            text = table.get_text()
            # 找包含任何球隊名的表格
            if not any(t in text for t in TEAM_ZH):
                continue
            headers = [th.get_text(strip=True) for th in table.find_all('th')]
            print(f"  → 找到球隊表格, headers: {headers}")
            ...
            for row in table.find_all('tr')[1:]:
                ...
                if not cols:
                    continue
                full = ' '.join(cols)
                team = next((t for t in TEAM_ZH if t in full), None)
                if not team:
                    continue
                # 找所有數字
                nums = re.findall(r'\b(\d+)\b', full)
                nums = [int(n) for n in nums if int(n) < 200]
                print(f"    {team}: cols={cols}, nums={nums}")

                # 解析勝-和-敗：CPBL 格式 "17-1-14" 或分開的欄位
                w = l = tie = 0
                # 試 "W-T-L" 格式
                wl_m = re.search(r'(\d+)-(\d+)-(\d+)', full)
                if wl_m:
                    w, tie, l = int(wl_m.group(1)), int(wl_m.group(2)), int(wl_m.group(3))
                elif len(nums) >= 2:
                    w, l = nums[0], nums[1]
                    tie = nums[2] if len(nums) > 2 else 0

                # 勝率
                pct_m = re.search(r'(0\.\d+|\.\d+)', full)
                pct_raw = pct_m.group(1) if pct_m else ''
                if pct_raw and not pct_raw.startswith('.'):
                    pct_raw = '.' + pct_raw.split('.')[1]

                # 差距
                gb_m = re.search(r'(\d+\.?\d*|-)\s*$', full.strip())
                gb = gb_m.group(1) if gb_m else '-'

                rows_data.append({'team': team, 'win': w, 'loss': l, 'draw': tie,
        ...
        if rows_data:
            ...
            print(f"  → 積分榜解析成功：{len(standings_override)} 隊")
            for s in standings_override:
                print(f"    {s['rank']}. {s['team']} {s['win']}-{s['draw']}-{s['loss']} {s['pct']}")
    ...
    try:
        ...
        for table in soup.find_all('table'):
            ...
            # 找比賽結果表格
            if not re.search(r'\d{1,2}/\d{1,2}', ttext):
                continue
            if not any(t in ttext for t in ['味全龍','統一獅','富邦悍將','台鋼雄鷹','樂天桃猿','中信兄弟']):
                continue
            for row in table.find_all('tr')[1:]:
                cols = [td.get_text(' ', strip=True) for td in row.find_all('td')]
                if len(cols) < 3:
                ...
                full = ' '.join(cols)
                # 跳過未來場次
                if any(kw in full for kw in ['尚未開打', '進行中', 'vs.', 'VS.']):
                    continue
                ...
                if a_s > 25 or h_s > 25:
                    continue
                teams = [norm_team(t) for t in ['味全龍','統一獅','富邦悍將','台鋼雄鷹','樂天桃猿','中信兄弟'] if t in full]
                if len(teams) < 2:
                    continue
