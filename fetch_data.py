...
def fetch_scores_for_standings():
    """
    積分榜：disp.cc PTT 戰績貼文（有平手/差距/連勝敗）
    近期比賽：LINE TODAY schedule 頁面（有多天比分，SSR）
    """
    results = []
    ...
    # ── 1. 積分榜：disp.cc ──
    try:
        soup = fetch_soup('https://disp.cc/b/Baseball/i3YR')
        text = soup.get_text('\n')
        lines = [l.strip() for l in text.splitlines() if l.strip()]

        all_links = re.findall(r'https://disp\.cc/b/Baseball/[a-zA-Z0-9]+', text)
        seen_l, unique = set(), []
        for lk in all_links:
            if lk not in seen_l:
                seen_l.add(lk); unique.append(lk)
        if unique and unique[-1] != 'https://disp.cc/b/Baseball/i3YR':
            soup = fetch_soup(unique[-1])
            text = soup.get_text('\n')
            lines = [l.strip() for l in text.splitlines() if l.strip()]
        ...
        for line in lines:
            ...
            if m:
                ...
                pct_fmt = '.' + pct.split('.')[-1] if '.' in pct else pct
                gb_fmt = '-' if rk == '1' or gb in ('0.0','0','-') else gb
                rows.append({'rank':int(rk),'team':team,'win':int(w),'draw':int(tie),
                             'loss':int(l2),'pct':pct_fmt,'gb':gb_fmt,'streak':strk})
        if rows:
            standings_override = sorted(rows, key=lambda x: x['rank'])
            print(f"  → 積分榜：{len(standings_override)} 隊")
    except Exception as e:
        print(f"  ⚠️ disp.cc 失敗: {e}")

    # ── 2. 近期比賽：LINE TODAY schedule（多月、有比分）──
    seen = set()
    ...
    for month_offset in range(0, 2):
        ...
        if m <= 0:
            m += 12; y -= 1
        url = f'https://today.line.me/tw/v3/baseball/seasons/{SEASON}/schedule?month={y}-{m:02d}'
        try:
            soup = fetch_soup(url)
            # LINE TODAY 賽程：找 <a href="/tw/v3/baseball/games/XXX">
            game_links = soup.find_all('a', href=re.compile(r'/tw/v3/baseball/games/'))
            for link in game_links:
                href = link.get('href', '')
                gid_m = re.search(r'/games/(\w+)', href)
                if not gid_m:
                    continue
                # 從 link 內文找日期、隊名、比分
                text_block = link.get_text(' ', strip=True)
                date_m2 = re.search(r'(\d{1,2})/(\d{1,2})', text_block)
                if not date_m2:
                    continue
                gd = f"{y}/{int(date_m2.group(1)):02d}/{int(date_m2.group(2)):02d}"
                if not is_past(gd) or gd < cutoff:
                    continue
                score_m = re.search(r'(\d{1,2})\s*[：:\-]\s*(\d{1,2})', text_block)
                if not score_m:
                    continue
                a_s, h_s = int(score_m.group(1)), int(score_m.group(2))
                if a_s > 25 or h_s > 25:
                    continue
                teams = [norm_team(t) for t in TEAM_LIST if t in text_block]
                if len(teams) < 2:
                    continue
                key = f"{gd}{teams[0]}{teams[1]}"
                if key not in seen:
                    seen.add(key)
                    results.append({'date':gd,'away':teams[0],'away_score':a_s,
                                    'home':teams[1],'home_score':h_s})

            # 若 link 方式抓不到，改用文字掃描
            if not results:
                text_all = soup.get_text('\n')
                page_lines = [l.strip() for l in text_all.splitlines() if l.strip()]
                cur_date = ''
                for i2, ln in enumerate(page_lines):
                    dm = re.fullmatch(r'(\d{1,2})/(\d{1,2})', ln)
                    if dm:
                        cur_date = f"{y}/{int(dm.group(1)):02d}/{int(dm.group(2)):02d}"
                    if cur_date and is_past(cur_date) and cur_date >= cutoff:
                        # 在附近行找比分 X:Y
                        window = ' '.join(page_lines[max(0,i2-3):i2+8])
                        sm2 = re.search(r'(\d{1,2})\s*[:：]\s*(\d{1,2})', window)
                        if not sm2:
                            continue
                        a_s2, h_s2 = int(sm2.group(1)), int(sm2.group(2))
                        if a_s2 > 25 or h_s2 > 25:
                            continue
                        teams2 = [norm_team(t) for t in TEAM_LIST if t in window]
                        if len(teams2) < 2:
                            continue
                        key2 = f"{cur_date}{teams2[0]}{teams2[1]}"
                        if key2 not in seen:
                            seen.add(key2)
                            results.append({'date':cur_date,'away':teams2[0],'away_score':a_s2,
                                            'home':teams2[1],'home_score':h_s2})

        except Exception as e:
            print(f"  ⚠️ LINE TODAY {y}/{m:02d} 失敗: {e}")

    # 補抓 atplayertw（backup）
    if not results:
        try:
            soup2 = fetch_soup('https://atplayertw.com.tw/cpbl/')
            tables = soup2.find_all('table')
            if len(tables) >= 2:
                for row in tables[1].find_all('tr')[1:]:
                    cols = [td.get_text(' ', strip=True) for td in row.find_all('td')]
                    if len(cols) < 4 or '尚未開打' in ' '.join(cols):
                        continue
                    date_m3 = re.search(r'(\d{1,2})/(\d{1,2})', cols[0])
                    if not date_m3:
                        continue
                    gd = f"{CUR_YEAR}/{int(date_m3.group(1)):02d}/{int(date_m3.group(2)):02d}"
                    if not is_past(gd):
                        continue
                    sm3 = re.search(r'(\d{1,2})\s*[：:]\s*(\d{1,2})', cols[2])
                    if not sm3:
                        continue
                    a_s3, h_s3 = int(sm3.group(1)), int(sm3.group(2))
                    if a_s3 > 25 or h_s3 > 25:
                        continue
                    away = norm_team(cols[1].strip())
                    home = norm_team(cols[3].strip())
                    key3 = f"{gd}{away}{home}"
                    if key3 not in seen:
                        seen.add(key3)
                        results.append({'date':gd,'away':away,'away_score':a_s3,'home':home,'home_score':h_s3})
        except Exception as e2:
            print(f"  ⚠️ atplayertw backup 失敗: {e2}")
