...
def fetch_standings():
    """從 CPBL 官網爬取目前積分榜與近期戰績"""
    standings = []
    recent = []

    try:
        r = get('https://www.cpbl.com.tw/standing')
        soup = BeautifulSoup(r.text, 'html.parser')

        # 積分榜表格
        for i, row in enumerate(soup.select('table tr')[1:], 1):
            cols = [td.get_text(strip=True) for td in row.select('td')]
            if len(cols) >= 5:
                standings.append({
                    'rank':   i,
                    'team':   cols[0],
                    'win':    int(cols[1]) if cols[1].isdigit() else 0,
                    'loss':   int(cols[2]) if cols[2].isdigit() else 0,
                    'draw':   int(cols[3]) if cols[3].isdigit() else 0,
                    'pct':    cols[4],
        ...
        # 近期比賽結果
        r2 = get('https://www.cpbl.com.tw/schedule/result')
        soup2 = BeautifulSoup(r2.text, 'html.parser')
        for game in soup2.select('.GameResult, [class*="result"]')[:15]:
            teams = re.findall(r'[\u4e00-\u9fff\w]+(?:兄弟|桃猿|獅|雄鷹|龍|悍將)', game.get_text())
            scores = re.findall(r'\b(\d{1,2})\b', game.get_text())
            date_m = re.search(r'(\d{4}/\d{2}/\d{2})', game.get_text())
