...
TARGET_VENUES = [
    ...
    'TICC','台北國際會議中心','台大綜合體育館','台大體育館',
    '林口體育館','桃園林口','台中洲際','台中流行音樂中心',
...
def fetch_scores_for_standings():
    """
    積分榜：disp.cc PTT 戰績貼文
    近期比賽：flashscore.com CPBL results（純 SSR，有完整比分）
    備用：AiScore CPBL scoreboard
    """
    results = []
    ...
    # ── 1. 積分榜：disp.cc（已確認可用）──
    KNOWN_ARTICLES = [
        # (date_str, url) — 每次有新貼文時 Actions 會自動找
        # 最新的放前面，積分榜取第一篇能解析的
    ]
    try:
        # 從已知貼文抓同主題列表
        soup0 = fetch_soup('https://disp.cc/b/Baseball/i4d5')
        text0 = soup0.get_text('\n')

        # 從文字中找「MM-DD ... CPBL戰績」+ URL
        articles = {}
        lines0 = text0.splitlines()
        for i, line in enumerate(lines0):
            if ('CPBL戰績' in line or 'CPBL 戰績' in line) and '二軍' not in line:
                # 找日期 MM-DD
                dm = re.search(r'(\d{2})-(\d{2})', line)
                # 找 URL（同行或前後）
                for j in range(max(0,i-2), min(len(lines0),i+3)):
                    um = re.search(r'/b/Baseball/([a-zA-Z0-9]+)', lines0[j])
                    if um and dm:
                        month, day = int(dm.group(1)), int(dm.group(2))
                        art_date = f"{CUR_YEAR}/{month:02d}/{day:02d}"
                        art_url = f"https://disp.cc/b/Baseball/{um.group(1)}"
                        articles[art_url] = art_date
                        break

        # 加入 HTML a 標籤找到的
        for a in soup0.find_all('a', href=re.compile(r'/b/Baseball/[a-zA-Z0-9]+')):
            art_url = 'https://disp.cc' + a['href']
            if art_url in articles:
                continue
            ctx = (a.parent.get_text() if a.parent else '') + line
            if ('CPBL戰績' in ctx or 'CPBL 戰績' in ctx) and '二軍' not in ctx:
                dm2 = re.search(r'(\d{2})-(\d{2})', ctx)
                if dm2:
                    art_date2 = f"{CUR_YEAR}/{int(dm2.group(1)):02d}/{int(dm2.group(2)):02d}"
                    articles[art_url] = art_date2

        # 種子本身
        articles['https://disp.cc/b/Baseball/i4d5'] = f"{CUR_YEAR}/05/19"

        # 動態找最新（板頁）
        try:
            soup_b = fetch_soup('https://disp.cc/b/Baseball')
            for a in soup_b.find_all('a', href=re.compile(r'/b/Baseball/[a-zA-Z0-9]+')):
                if 'CPBL戰績' in a.get_text() or (a.parent and 'CPBL戰績' in a.parent.get_text()):
                    new_url = 'https://disp.cc' + a['href']
                    # 找日期
                    dm3 = re.search(r'(\d{2})-(\d{2})', a.parent.get_text() if a.parent else '')
                    if dm3:
                        articles[new_url] = f"{CUR_YEAR}/{int(dm3.group(1)):02d}/{int(dm3.group(2)):02d}"
                    break
        except:
            pass

        print(f"  → 找到 {len(articles)} 篇戰績貼文")

        # 解析每篇
        import datetime as dt
        cutoff = (TODAY - dt.timedelta(days=10)).strftime('%Y/%m/%d')
        for url, date_str in sorted(articles.items(), key=lambda x: x[1], reverse=True):
            if date_str < cutoff:
                continue
            try:
                soup1 = fetch_soup(url)
                for line in [l.strip() for l in soup1.get_text('\n').splitlines() if l.strip()]:
                    # 積分榜
                    m = re.match(
                        r'^(\d)\s+(味全龍|統一獅|富邦悍將|台鋼雄鷹|樂天桃猿|中信兄弟)\s+'
                        r'\d+\s+(\d+)-(\d+)-(\d+)\s+([\d.]+)\s+([\d.]+|-)\s+([WL]\d+)', line)
                    if m and not standings_override:
                        rk,team,w,tie,l2,pct,gb,strk = m.groups()
                        pf = '.' + pct.split('.')[-1] if '.' in pct else pct
                        gf = '-' if rk=='1' or gb in ('0.0','0','-') else gb
                        if standings_override is None:
                            standings_override = []
                        if not any(r['team']==team for r in standings_override):
                            standings_override.append(
                                {'rank':int(rk),'team':team,'win':int(w),'draw':int(tie),
                                 'loss':int(l2),'pct':pf,'gb':gf,'streak':strk})

                    # 比賽結果
                    gm = re.match(
                        r'^(味全龍|統一獅|富邦悍將|台鋼雄鷹|樂天桃猿|中信兄弟)\s+'
                        r'(\d{1,2})[：:]\s*(\d{1,2})\s+'
                        r'(味全龍|統一獅|富邦悍將|台鋼雄鷹|樂天桃猿|中信兄弟)', line)
                    if gm and date_str:
                        away,a_s,h_s,home = gm.group(1),int(gm.group(2)),int(gm.group(3)),gm.group(4)
                        if a_s<=25 and h_s<=25:
                            key = f"{date_str}{away}{home}"
                            if key not in seen:
                                seen.add(key)
                                results.append({'date':date_str,'away':away,'away_score':a_s,
                                                'home':home,'home_score':h_s})
            except Exception as e:
                print(f"    ⚠️ {url}: {e}")

        if standings_override:
            standings_override = sorted(standings_override, key=lambda x: x['rank'])
            print(f"  → 積分榜：{len(standings_override)} 隊")

    except Exception as e:
        print(f"  ⚠️ disp.cc 失敗: {e}")

    # ── 2. 近期比賽：flashscore（若 disp.cc 比賽不夠）──
    if len(results) < 5:
        try:
            # flashscore CPBL results 頁面（SSR）
            r = requests.get(
                'https://www.flashscore.com/baseball/taiwan/cpbl/results/',
                headers={**HEADERS, 'Accept': 'text/html'},
                timeout=15)
            if r.status_code == 200:
                soup_fs = BeautifulSoup(r.text, 'html.parser')
                text_fs = soup_fs.get_text('\n')
                # 找比分行（格式多樣，先 print 看看）
                lines_fs = [l.strip() for l in text_fs.splitlines() if l.strip()]
                print(f"  → flashscore 取得 {len(lines_fs)} 行")
                # 找包含台灣隊名的行
                for line in lines_fs[:200]:
                    gm = re.search(
                        r'(味全龍|統一獅|富邦悍將|台鋼雄鷹|樂天桃猿|中信兄弟).{0,20}'
                        r'(\d{1,2})[：:\s]+(\d{1,2}).{0,20}'
                        r'(味全龍|統一獅|富邦悍將|台鋼雄鷹|樂天桃猿|中信兄弟)', line)
                    if gm:
                        print(f"    flashscore hit: {line}")
        except Exception as e:
            print(f"  ⚠️ flashscore 失敗: {e}")

    print(f"  → 近期比賽共 {len(results)} 場")
...
def main():
    ...
    if standings_override:
        ...
        print(f"  → 積分榜 OK（{len(standings)} 隊）")
    else:
        standings = calc_standings(games_past + game_results)
