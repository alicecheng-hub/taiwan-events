...
def fetch_scores_for_standings():
    """
    從 disp.cc CPBL戰績貼文抓取積分榜和近期比賽。
    確認格式（5/27貼文）:
      積分榜: 「1 味全龍 41 27-0-14 0.659 0.0 L1 M17」
      比賽結果: 「富邦悍將 5:1 味全龍」
    """
    ...
    def parse_article(url, art_date):
        ...
        try:
            ...
            for line in [l.strip() for l in soup.get_text('\n').splitlines() if l.strip()]:
                # 積分榜行（格式：「1 味全龍 41 27-0-14 0.659 0.0 L1」）
    ...
    # 已知近期貼文（從搜尋確認的，按日期新到舊）
    # 每天 nybbnan 發，URL 碼每次不同，只能用種子往前找
    known = {
        'https://disp.cc/b/Baseball/i4To': f"{CUR_YEAR}/05/27",
        'https://disp.cc/b/Baseball/i4d5': f"{CUR_YEAR}/05/19",
    }
    ...
    try:
        ...
        for a in soup_b.find_all('a', href=re.compile(r'/b/Baseball/[a-zA-Z0-9]+')):
            ...
            combined = txt + parent_txt
            if 'CPBL戰績' in combined and '二軍' not in combined:
                art_url = 'https://disp.cc' + a['href']
                dm = re.search(r'(\d{2})-(\d{2})', parent_txt)
                if dm:
                    art_date = f"{CUR_YEAR}/{int(dm.group(1)):02d}/{int(dm.group(2)):02d}"
                    known[art_url] = art_date
    except Exception as e:
        print(f"  ⚠️ 板頁: {e}")

    # 從最新種子貼文找同主題連結（往前追）
    for seed_url in list(known.keys())[:1]:
        try:
            soup_s = fetch_soup(seed_url)
            lines_s = soup_s.get_text('\n').splitlines()
    ...
    print(f"  → 找到 {len(known)} 篇戰績貼文")

    # 逐篇解析，最新的先
    for url, art_date in sorted(known.items(), key=lambda x: x[1], reverse=True):
        ...
        if rows and not standings_done:
            ...
            if len(all_rows) >= 6:
                standings_done = True
                print(f"  → 積分榜：{len(all_rows)} 隊（{url}）")
