...
# 球隊 logo 來源（Wikipedia，Actions 環境可以連）
TEAM_LOGO_URLS = {
    '中信兄弟': 'https://upload.wikimedia.org/wikipedia/zh/5/5e/CTBC_Brothers_Logo.svg',
    '統一獅':   'https://upload.wikimedia.org/wikipedia/zh/e/e4/Uni-President_7-Eleven_Lions_logo.svg',
    '樂天桃猿': 'https://upload.wikimedia.org/wikipedia/zh/e/e0/Rakuten_Monkeys_logo.svg',
    '富邦悍將': 'https://upload.wikimedia.org/wikipedia/zh/a/a7/Fubon_Guardians_logo.svg',
    '味全龍':   'https://upload.wikimedia.org/wikipedia/zh/5/54/Wei_Chuan_Dragons_logo.svg',
    '台鋼雄鷹': 'https://upload.wikimedia.org/wikipedia/zh/2/21/TSG_Hawks_logo.svg',
}
LOGO_FILENAMES = {
    '中信兄弟': 'img/brothers.svg',
    '統一獅':   'img/lions.svg',
    '樂天桃猿': 'img/monkeys.svg',
    '富邦悍將': 'img/guardians.svg',
    '味全龍':   'img/dragons.svg',
    '台鋼雄鷹': 'img/hawks.svg',
}
        ...
        try:
            ...
            if r.status_code == 200 and len(r.content) > 500:
                with open(path, 'wb') as f:
                    f.write(r.content)
                ok += 1
                print(f"  ✓ {team} logo 下載成功")
            else:
                print(f"  ⚠️ {team} logo 失敗: {r.status_code}")
        except Exception as e:
            print(f"  ⚠️ {team} logo 例外: {e}")
    print(f"  → Logo: {ok}/{len(TEAM_LOGO_URLS)} 個")
...
def norm_venue(v):
    v = v.strip()
    if not v or v in ('待定', '有待公佈', '待公布', 'TBA'): return '待公布'
...
def is_large_venue(venue):
    return any(kw in venue for kw in LARGE_VENUES)
...
def fetch_scores_for_standings():
    """
    atplayertw 真實結構（已確認）:
    table[0]: 球隊, W, L, PCT, GB, 得, 失, 分差, 連勝敗, 近況
    table[1]: 台灣時間, 客隊, 比分, 主隊, 狀態, 場地
    """
    results = []
    ...
    try:
        ...
        if tables:
            ...
            for row in tables[0].find_all('tr')[1:]:
                ...
                if len(cols) < 4:
                    continue
                team = next((t for t in TEAM_SHORT if t in cols[0]), None)
                if not team:
                    continue
                try:
                    w = int(re.search(r'\d+', cols[1]).group())
                    l = int(re.search(r'\d+', cols[2]).group())
                    pct_raw = '.' + cols[3].split('.')[-1]
                    gb = cols[4].strip() if len(cols) > 4 else '-'
                    # 連勝敗欄（col[8]）格式：'勝2' or '敗1'
                    streak = ''
                    if len(cols) > 8:
                        m = re.match(r'(勝|敗)(\d+)', cols[8].strip())
                        if m:
                            streak = f"{'W' if m.group(1)=='勝' else 'L'}{m.group(2)}"
                    # 近況欄（最後）格式：'LWWWL'
                    if not streak and cols:
                        near = cols[-1].strip()
                        if re.fullmatch(r'[WL]+', near) and len(near) >= 2:
                            last = near[-1]
                            cnt = sum(1 for c in reversed(near) if c == last)
                            streak = f"{'W' if last=='W' else 'L'}{min(cnt,5)}"
                except:
                    continue
                rows_data.append({'team': team, 'win': w, 'loss': l, 'draw': 0,
                                  'pct': pct_raw, 'gb': gb, 'streak': streak})

            if rows_data:
                rows_data.sort(key=lambda x: (-x['win'], x['loss']))
                standings_override = [dict(r, rank=i+1) for i, r in enumerate(rows_data)]
        ...
        if len(tables) >= 2:
            for row in tables[1].find_all('tr')[1:]:
                ...
                if not is_past(gd):
                    continue
                score_m = re.search(r'(\d{1,2})\s*[：:]\s*(\d{1,2})', cols[2])
                if not score_m:
                    continue
                ...
                if key not in seen:
                    seen.add(key)
                    results.append({'date': gd, 'away': away, 'away_score': a_s,
...
def fetch_concerts():
    ...
    for page in range(1, 11):
        ...
        for link in links:
            ...
            # 演出日期 - 抓所有日期（多天演出）
            all_dates = re.findall(r'(\d{4}年\d{1,2}月\d{1,2}日)', block)
            if not all_dates:
                continue
            date_str = parse_tw_date(all_dates[0])
            if not date_str or not is_future(date_str):
                continue
            date_end = parse_tw_date(all_dates[-1]) if len(all_dates) > 1 else ''

            venue_m = re.search(r'演出場館[:：]\s*([\S ]+?)(?:門票|全面|優先|⏰|🔔|\n|$)', block)
            venue = norm_venue(venue_m.group(1).strip() if venue_m else '')
    ...
    for title, item in scraped.items():
        if title in merged:
            ...
            if item.get('date_end'):
                merged[title]['date_end'] = item['date_end']
            update_count += 1
        else:
            merged[title] = item
            new_count += 1
