...
# CPBL box score 每場的起始 gameSno（每年1月開始），當月大概是多少
# 每月約 70-90 場，5月約從 gameSno 70 開始
CPBL_GAME_SNO_START = 60   # 保守估計，會往前後各試
...
def fetch_scores_for_standings():
    """
    從 CPBL 官網 box score 頁面逐場抓比分。
    URL: https://www.cpbl.com.tw/box?year=YYYY&kindCode=A&gameSno=N
    用你截圖給我的那個 URL 格式，每場一頁，SSR 純 HTML。
    策略：從 gameSno=1 往上試，碰到今天之前的比賽就收集比分。
    """
    results = []
    seen = set()
    ...
    # 找今年已打完的場次範圍
    # 從 gameSno=1 開始，連續失敗超過 5 場就停
    consecutive_fail = 0
    for sno in range(1, 200):
        ...
        # 找比分：頁面上會有 "X : Y" 或 "客隊X 主隊Y" 的格式
        score_m = re.search(r'(\d{1,2})\s*[：:]\s*(\d{1,2})', page_text)
        if not score_m:
            # 備用：找頁面 title 裡的比分
            title = soup.title.get_text() if soup.title else ''
            score_m = re.search(r'(\d{1,2})\s*[：:]\s*(\d{1,2})', title)
        if not score_m:
            continue
        ...
        if key not in seen:
            ...
            results.append({
                'date': game_date,
                'away': teams[0], 'away_score': away_score,
                'home': teams[1], 'home_score': home_score,
            })
            print(f"  ✓ {game_date} {teams[0]} {away_score}:{home_score} {teams[1]}")

    print(f"  → CPBL box score 共抓到 {len(results)} 場比賽結果")

    # 若完全失敗（被擋），fallback 到文字掃描
    if not results:
        print("  → 改用 LINE TODAY 文字掃描...")
...
def _fetch_scores_line_today():
    ...
    for i, l in enumerate(lines):
        if re.fullmatch(r'\d{1,2}/\d{1,2}', l):
            p = l.split('/')
            cur_date = f"{CUR_YEAR}/{int(p[0]):02d}/{int(p[1]):02d}"
        if cur_date and is_past(cur_date):
            ...
            if len(teams) == 2 and len(scores) >= 2:
                ...
                if not any(r.get('_key') == key for r in results):
                    results.append({'_key': key, 'date': cur_date,
