...
TARGET_VENUES = [
    ...
    '台北小巨蛋','小巨蛋',
    '高雄巨蛋',
    '高雄世運','世運主場館','世運','高雄國家體育場',
    '高雄流行音樂中心','海音館',
    '台北流行音樂中心',
    'TICC','台北國際會議中心',
    '台大綜合體育館','國立臺灣大學綜合體育館','台大體育館',
    '桃園林口體育館','林口體育館',
    '台中洲際','台中流行音樂中心',
    'Legacy','Legacy TERA','Legacy Taipei',
    'Zepp','Zepp New Taipei',
    '巨蛋','體育館','音樂中心',
]
...
def fetch_scores_for_standings():
    """
    從 CPBL 官網 /schedule/index 頁面抓已完賽的比分。
    CPBL 官網的賽程頁面是 SSR，純 HTML 即可解析，不需要 JS。
    每場比賽的結構：隊名 + 比分都在同一個 .GameBox 區塊內。
    """
    results = []
    seen = set()
    ...
    for month_offset in [0, -1]:
        ...
        url = f'https://www.cpbl.com.tw/schedule/index?year={y}&month={m:02d}&kindCode=A'
        try:
            r = requests.get(url, headers=HEADERS, timeout=20)
            r.raise_for_status()
        except Exception as e:
            print(f"  ⚠️ CPBL schedule {y}/{m:02d} 失敗: {e}")
            continue

        soup = BeautifulSoup(r.text, 'html.parser')

        # CPBL 官網結構：每個比賽日是 .GameDate，底下有多個 .GameBox
        cur_date = ''
        for elem in soup.find_all(['div', 'li', 'span', 'td'], recursive=True):
            cls = ' '.join(elem.get('class', []))

            # 找日期 header
            if 'GameDate' in cls or 'game-date' in cls.lower():
                d_text = elem.get_text(strip=True)
                m_obj = re.search(r'(\d{4})[/-](\d{1,2})[/-](\d{1,2})', d_text)
                if m_obj:
                    cur_date = f"{m_obj.group(1)}/{int(m_obj.group(2)):02d}/{int(m_obj.group(3)):02d}"
                    continue
                # 有時只有 M/D 格式
                m_obj2 = re.search(r'(\d{1,2})/(\d{1,2})', d_text)
                if m_obj2:
                    cur_date = f"{y}/{int(m_obj2.group(1)):02d}/{int(m_obj2.group(2)):02d}"

            # 找比賽結果區塊（已完賽才有比分）
            if ('GameBox' in cls or 'game-box' in cls.lower()) and cur_date and is_past(cur_date):
                text = elem.get_text(' ', strip=True)
                # 找隊名
                teams = []
                for t in TEAM_LIST:
                ...
                scores = [int(s) for s in scores if int(s) <= 30]
        ...
        print(f"  → CPBL {y}/{m:02d} 抓到 {len(results)} 場已完賽結果")

    # 若 CPBL 官網失敗（全被擋），改從 LINE TODAY 文字掃描（原本邏輯）
    if not results:
        print("  → CPBL 官網無結果，改用 LINE TODAY 文字掃描...")
        results = _fetch_scores_line_today()

    return results


def _fetch_scores_line_today():
    """備用：LINE TODAY 文字掃描（原本邏輯，比分通常抓不到）"""
    url = f'https://today.line.me/tw/v3/baseball/seasons/{SEASON}/schedule'
    try:
        soup = fetch_soup(url)
        text = soup.get_text('\n')
...
def fetch_concerts():
    ...
    # 從 tickettw 抓新資料（共10頁，URL 格式為 /concerts 和 /concerts/N）
    scraped = {}
