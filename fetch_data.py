...
def fetch_scores_for_standings():
    ...
    known = {
        ...
        'https://disp.cc/b/Baseball/i4wJ': f"{CUR_YEAR}/05/23",
        ...
        for a in soup_b.find_all('a', href=re.compile(r'/b/Baseball/[a-zA-Z0-9]+')):
            ...
            if 'CPBL戰績' in ctx and '二軍' not in ctx:
                ...
                if dm:
                    d = f"{CUR_YEAR}/{int(dm.group(1)):02d}/{int(dm.group(2)):02d}"
                    known[art_url] = d
                    print(f"  → 板頁找到最新: {art_url} ({d})")
                break
    except Exception as e:
        print(f"  ⚠️ 板頁: {e}")

    # 從最新貼文追同主題連結
    for url, d in sorted(known.items(), key=lambda x: x[1], reverse=True)[:1]:
        _, _, extra = parse_one(url, d)
        known.update(extra)

    print(f"  → 找到 {len(known)} 篇戰績貼文: {sorted(known.values(), reverse=True)}")
