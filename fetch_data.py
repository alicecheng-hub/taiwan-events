...
def fetch_scores_for_standings():
    ...
    這個網站不擋 GitHub Actions IP，純 SSR HTML 可直接解析。
    ...
    try:
        ...
        for table in soup.find_all('table'):
            ...
            for row in table.find_all('tr')[1:]:
                ...
                try:
                    ...
                    continue
                rows.append({'team':team,'win':w,'loss':l,'draw':0,'pct':pct_s,'gb':gb,'streak':streak})
        ...
        # ── 比賽結果 ──
        for table in soup.find_all('table'):
            if '客隊' not in table.get_text() and '比分' not in table.get_text():
                continue
            for row in table.find_all('tr')[1:]:
                cols = [td.get_text(' ', strip=True) for td in row.find_all('td')]
                if len(cols) < 3:
                    continue
                full = ' '.join(cols)
                if '尚未開打' in full or '進行中' in full:
                    continue
                ...
                if not is_past(gd):
                    continue
                score_m = re.search(r'(\d{1,2})\s*[：:]\s*(\d{1,2})', full)
                if not score_m:
                    continue
                teams = []
                for t in TEAM_LIST:
                ...
                if key not in seen:
                    ...
                    results.append({
                        'date': gd,
                        'away': teams[0], 'away_score': int(score_m.group(1)),
                        'home': teams[1], 'home_score': int(score_m.group(2)),
                    })
...
def main():
    ...
    # 近期結果：優先用 atplayertw 的（有真實比分），否則用 games_past
    all_results = game_results if game_results else games_past
