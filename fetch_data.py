...
def fetch_scores_for_standings():
    ...
    近期比賽：CPBL 官網 box score 逐場抓（cpbl.com.tw/box?gameSno=N）
              gameSno 從 games_past 裡的場次往回推，或從已知範圍掃描
    """
    results = []
    ...
    # ── 2. 近期比賽：CPBL box score（逐場，最近 10 天）──
    import datetime as dt
    cutoff = (TODAY - dt.timedelta(days=10)).strftime('%Y/%m/%d')
    seen = set()

    # 從今年已知大概的場次範圍掃描（今年到今天大約 100-120 場）
    # 每月約 70 場，5月中旬大約 sno=100~120
    # 用二分方式：先試 sno=80~140
    session = requests.Session()
    session.headers.update(HEADERS)
    try:
        session.get('https://www.cpbl.com.tw/', timeout=10)
    except:
        pass

    # 找今天之前最近幾場（從 sno=80 往上，碰到未來就停）
    hit_future = False
    for sno in range(80, 160):
        if hit_future:
            break
        url = f'https://www.cpbl.com.tw/box?year={CUR_YEAR}&kindCode=A&gameSno={sno}'
        try:
            ...
            # 找日期
            date_m = re.search(r'(\d{4})[/-](\d{2})[/-](\d{2})', page_text)
            if not date_m:
                continue
            gd = f"{date_m.group(1)}/{date_m.group(2)}/{date_m.group(3)}"
            ...
            # 找比分（格式 "X : Y" 在成績看板）
            score_m = re.search(r'\b(\d{1,2})\s*[：:]\s*(\d{1,2})\b', page_text)
            if not score_m:
                continue
