...
def fetch_scores_for_standings():
    """
    從 atplayertw.com.tw/cpbl/ 抓積分榜和近期比賽結果（含真實比分）。
    同時回傳 (game_results, standings_override)。
    """
    results = []
    ...
    TEAM_EN_MAP = {
        '味全龍':'味全龍','Wei Chuan':'味全龍',
        '統一獅':'統一獅','Uni Lions':'統一獅','Uni ':'統一獅',
        '富邦悍將':'富邦悍將','Fubon':'富邦悍將',
        '台鋼雄鷹':'台鋼雄鷹','TSG':'台鋼雄鷹',
        '樂天桃猿':'樂天桃猿','Rakuten':'樂天桃猿',
        '中信兄弟':'中信兄弟','Chinatrust':'中信兄弟',
    }
    ...
    try:
        ...
        for table in soup.find_all('table'):
            ...
            for row in table.find_all('tr')[1:]:
                ...
                try:
                    ...
                    streak_raw = cols[8].strip() if len(cols) > 8 else ''
                    streak = ''
                    if streak_raw:
                        last = streak_raw[-1]
                        cnt = sum(1 for c in reversed(streak_raw) if c == last)
                        streak = f"{'W' if last=='W' else 'L'}{min(cnt,5)}"
                except:
                    continue
                rows.append({'team':team,'win':w,'loss':l,'draw':0,
                             'pct':pct_s,'gb':gb,'streak':streak})
            if rows:
        ...
        # ── 近期比賽結果（過去7天）──
        cutoff = (TODAY - __import__('datetime').timedelta(days=7)).strftime('%Y/%m/%d')
...
def main():
    ...
    # 近期結果：用 atplayertw 的（有真實比分），最多15筆，過去7天
