...
def fetch_scores_for_standings():
    """
    從 disp.cc CPBL戰績貼文抓積分榜和近期比賽。
    nybbnan 每天發，格式：
      積分榜: 「1 味全龍 41 27-0-14 0.659 0.0 L1」
      比賽: 「富邦悍將 5:1 味全龍」
    """
    ...
    def parse_one(url, art_date):
        ...
        try:
            ...
            # 從同主題連結找更多文章
            extra = {}
            for a in soup.find_all('a', href=re.compile(r'/b/Baseball/[a-zA-Z0-9]+')):
                art_url = 'https://disp.cc' + a['href']
                parent_txt = (a.parent.get_text() if a.parent else '') + a.get_text()
                if ('CPBL戰績' in parent_txt or 'CPBL 戰績' in parent_txt) and '二軍' not in parent_txt:
                    dm = re.search(r'(\d{2})-(\d{2})', parent_txt)
                    if dm:
                        d = f"{CUR_YEAR}/{int(dm.group(1)):02d}/{int(dm.group(2)):02d}"
                        if d >= cutoff:
                            extra[art_url] = d
        except Exception as e:
            print(f"    ⚠️ {url}: {e}")
        return rows, games, extra

    # 已知近期貼文（搜尋確認）
    known = {
        'https://disp.cc/b/Baseball/i4To': f"{CUR_YEAR}/05/27",  # 5/27
        'https://disp.cc/b/Baseball/i4wJ': f"{CUR_YEAR}/05/23",  # 5/23
        'https://disp.cc/b/Baseball/i4d5': f"{CUR_YEAR}/05/19",  # 5/19
    }
    ...
    try:
        ...
        for a in soup_b.find_all('a', href=re.compile(r'/b/Baseball/[a-zA-Z0-9]+')):
            ...
            ctx = (a.parent.get_text() if a.parent else '') + txt
            if 'CPBL戰績' in ctx and '二軍' not in ctx:
                art_url = 'https://disp.cc' + a['href']
                dm = re.search(r'(\d{2})-(\d{2})', ctx)
                if dm:
                    d = f"{CUR_YEAR}/{int(dm.group(1)):02d}/{int(dm.group(2)):02d}"
                    known[art_url] = d
                break
    ...
    # 從最新貼文追同主題連結
    sorted_known = sorted(known.items(), key=lambda x: x[1], reverse=True)
    if sorted_known:
        _, _, extra = parse_one(sorted_known[0][0], sorted_known[0][1])
        known.update(extra)
    ...
    for url, art_date in sorted(known.items(), key=lambda x: x[1], reverse=True):
        rows, games, _ = parse_one(url, art_date)
        all_games.extend(games)
        if rows and not standings_done:
            all_rows.extend(rows)
            if len(all_rows) >= 6:
                standings_done = True
                print(f"  → 積分榜：{len(all_rows)} 隊")
